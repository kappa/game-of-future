...
field in `session.md`. Translate these strings to the session language.
...
- `gof-product-card__core-idea`: **Write 3–4 sentences in plain language for a
...
- `gof-product-card__intersection`: `<strong>Why it works:</strong>` followed
  by one sentence in plain language explaining why the product depends on both
